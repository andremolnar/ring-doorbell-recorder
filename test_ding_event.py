...
from src.core.interfaces import EventData, DingEventData


async def main():
    """Simulate a doorbell ding event and test video recording."""
    logger.info("Starting ding event simulation...")
    ...
    event_id = f"test_ding_{timestamp}"
    ...
    try:
        ...
        # Inspect event_types dictionary in the capture engine
        logger.info(f"CaptureEngine event types mapping: {capture_engine._event_types}")
        
        # Create a simulated ding event
        ...
        event_data = {
            ...
            "answered": False  # Specific field for ding events
        }
        
        # First, explicitly create and save the event in storage
        logger.info(f"Creating ding event with ID: {event_id}")
        event_obj = DingEventData(
            id=event_id,
            kind="ding",
            created_at=datetime.datetime.now().isoformat(),
            device_id=device_id,
        ...
        # Wait for recording and processing to complete (ding events are longer than motion events)
        logger.info("Waiting 35 seconds for recording and processing to complete...")
        await asyncio.sleep(35)
        
        # Check if video was created in the event directory
        logger.info("Checking for recorded video in the ding event directory...")
        
        # Path pattern where the video should be stored
        video_path_base = os.path.join(storage_path, device_id, "ding", event_id)
                ...
                if mp4_files:
                    ...
                    try:
                        ...
                        if event:
                            ...
                            updated_event = DingEventData(**event_dict)
